...
function getChatName(chat) {
  ...
  return chat.pin ? chat.pin.slice(0, 10) : 'chat'
}
...
export default function AppHome() {
  ...
  // ── Panel ──────────────────────────────────────────────────────────────────
  const togglePanel = () => { setPanelOpen(v => !v); setDropdownOpen(false); setCreated(null); setPinError('') }
  ...
  const copyText = async (text, setFn) => {
    ...
    setFn(true); setTimeout(() => setFn(false), 1800)
  }
  ...
  const handleCreateLegacy = async (withPassword = false) => {
    ...
    let passwordHash = null, pw = null
    if (withPassword) { pw = generatePassword(); passwordHash = await hashPassword(pw) }
  ...
  const openChat = chat => {
    ...
    router.push(chat.type === 'pin-link' && chat.pin
      ? `/room/${chat.roomId}#${chat.pin}`
      : `/room/${chat.roomId}`)
  }
    ...
    <div style={container}>
      ...
      <div style={header}>
        <span
          ...
          onMouseDown={startLongPress} onMouseUp={clearLongPress}
          onMouseLeave={clearLongPress} onTouchStart={startLongPress} onTouchEnd={clearLongPress}
          title="hold to copy link"
        >
          {logoCopied ? 'copied!' : 'whispr'}
        </span>
        <button onClick={togglePanel} style={headerBtn}>{panelOpen ? '×' : '+'}</button>
      </div>
        ...
        <div style={panel}>
          <div style={panelRow}>
            ...
            <div style={{ position: 'relative' }} ref={dropdownRef}>
              <button onClick={() => setDropdownOpen(v => !v)} style={panelBtn}>⋯</button>
              {dropdownOpen && (
                <div style={dropdown}>
                  <label style={dropdownCheck}>
                    <input type="checkbox" checked={passwordProtect} onChange={togglePasswordProtect}
          ...
          {created && (
            <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
              {created.password && (
                <div style={pwRow}><span style={{ ...pwText, fontSize: 11 }}>pw: {created.password}</span></div>
              )}
              <div style={pwRow}>
        ...
        {chats.length === 0 ? (
          ...
          chats.map(chat => (
            <div key={chat.roomId} style={chatItem} onClick={() => openChat(chat)}>
              ...
              <span style={chatPreview}>
                {chat.lastMessage || (chat.type === 'ecdh' ? '2-party · ecdh' : chat.type === 'pin-link' ? 'link chat' : 'pin chat')}
              </span>
            </div>
          ))
        )}
...
const container = { minHeight: '100dvh', display: 'flex', flexDirection: 'column', background: '#111', color: '#ccc', fontFamily: 'monospace' }
const header = { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 20px 14px', borderBottom: '1px solid #1a1a1a', position: 'sticky', top: 0, background: '#111', zIndex: 10 }
const logo = { fontSize: 20, fontWeight: 400, letterSpacing: 3, cursor: 'default', userSelect: 'none', WebkitUserSelect: 'none' }
const headerBtn = { background: 'none', border: 'none', color: '#555', fontFamily: 'monospace', fontSize: 22, cursor: 'pointer', padding: '0 0 0 8px', lineHeight: 1 }
const panel = { padding: '12px 16px 14px', borderBottom: '1px solid #1a1a1a', display: 'flex', flexDirection: 'column', gap: 0 }
const panelRow = { display: 'flex', alignItems: 'center', gap: 6 }
const panelInput = { flex: 1, background: '#1a1a1a', border: 'none', borderRadius: 10, padding: '9px 12px', color: '#ccc', fontFamily: 'monospace', fontSize: 14, outline: 'none', minWidth: 0, letterSpacing: 1 }
const panelBtn = { background: 'none', border: 'none', color: '#555', fontFamily: 'monospace', fontSize: 18, cursor: 'pointer', padding: '6px 4px', lineHeight: 1, flexShrink: 0 }
const dropdown = { position: 'absolute', right: 0, top: 'calc(100% + 6px)', background: '#1a1a1a', borderRadius: 10, padding: '6px 0', minWidth: 230, zIndex: 100, boxShadow: '0 4px 20px rgba(0,0,0,0.5)' }
const dropdownCheck = { display: 'flex', alignItems: 'center', padding: '9px 14px', fontSize: 12, color: '#888', cursor: 'pointer', userSelect: 'none', WebkitUserSelect: 'none' }
const dropdownDivider = { height: 1, background: '#252525', margin: '4px 0' }
const dropdownBtn = { display: 'block', width: '100%', background: 'none', border: 'none', padding: '9px 14px', color: '#777', fontFamily: 'monospace', fontSize: 12, cursor: 'pointer', textAlign: 'left' }
const pwRow = { display: 'flex', alignItems: 'center', gap: 10, background: '#1a1a1a', borderRadius: 10, padding: '8px 12px', justifyContent: 'space-between', marginTop: 8 }
const pwText = { fontSize: 13, color: '#888', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }
const smallBtn = { background: 'none', border: '1px solid #2e2e2e', borderRadius: 6, padding: '3px 9px', color: '#555', fontFamily: 'monospace', fontSize: 11, cursor: 'pointer', flexShrink: 0 }
const openBtn = { width: '100%', background: '#1a1a1a', border: 'none', borderRadius: 10, padding: '11px 0', color: '#888', fontFamily: 'monospace', fontSize: 13, cursor: 'pointer', letterSpacing: 1 }
const chatList = { flex: 1, overflowY: 'auto' }
const empty = { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, height: '40vh' }
const chatItem = { padding: '14px 20px', borderBottom: '1px solid #161616', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 4 }
...
const chatPreview = { fontSize: 12, color: '#444', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }
